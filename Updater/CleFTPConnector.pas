...
type
  ...
  TMsgNotify  = procedure( stMsg : string ) of object;
  ...
  public
    ...
    property OnLog  : TMsgNotify write FOnLog;
...
begin
  ...
  if Assigned( FOnLog ) then
    FOnLog( 'Ftp Connected : ' + FServerIP );
...
begin
  ...
  try
    ...
    begin
      ...
      try
        ...
        begin
          ...
          begin
            if Assigned( FOnLog ) then
              FOnLog(aFile.Name + ' does not need to synchronize. Skipped.');

            Continue;
          end;

          if Assigned( FOnLog ) then
            FOnLog('Downloading ' + aFile.Name);
        ...
        begin
          if Assigned( FOnLog ) then
            FOnLog(aFile.Name + ' does not exist. Skipped');
        end;
      except
        on E : Exception do
        begin
          if Assigned( FOnLog ) then
            FOnLog('Downloading failed for ' + aFile.Name + E.Message);
        end;
      end;

      FFTP.Disconnect;
      if Assigned( FOnLog ) then
        FOnLog( 'FTP Dissconnected');
    end else
    begin
      Result := -1;
      FLastErrorMsg := 'connection or login failure';
    ...
    begin
      ...
      FLastErrorMsg := E.Message;
      if Assigned( FOnLog ) then
        FOnLog('Downloading failed for ' + FLastErrorMsg);
    end;
  end;
end;
