...
type
  ...
  TDerMaster =  record
     ...
     prdID : array[0..9] of char;                             //파생상품ID(상품구분코드)
     ...
     monthcode : char;                                        //월물구분코드 1:최근월물,선물스프레드 2:2째월물 3:3째월물 4:4째월물 5:5째월물 6:6째월물 7:7째월물
     ...
     underlyivcode : char;                                    //기초자산최대변동폭유형코드
     underlyiv : array[0..5] of char;                         //기초자산최대변동폭
     ...
     unsettledmaxcon : array[0..7] of char;                   //미결제 한도 계약수
     attachedprdt : array[0..3] of char;                      //소속 상품군
     prdtoptrate : array[0..4] of char;                       //상품군 옵셋율
     hogatype : char;                                         //지정가호가조건구분코드
     mhogatype : char;                                        //시장가호가조건구분코드
     chogatype : char;                                        //조건부지정가호가조건구분코드
     ahogatype : char;                                        //최유리지정가호가조건구분코드
  ...
  TStockMaster = record
    ...
    inducode : array[0..5] of char;                           //표준산업코드
    ...
    iscreditorder : char;                                     //신용주문가능여부
    limitcode : char;                                         //지정가호가조건구분코드
    marketcode : char;                                        //시장가호가조건구분코드
    conditionlimitcode : char;                                //조건부지정가호가조건구분
    bestcode : char;                                          //최유리지정가호가조건구분
    firstcode : char;                                         //최우선지정가호가조건구분
    incapital : array[0..1] of char;                          //증자구분코드
    firststockcode : char;                                    //우선주구분코드
    ispeople : char;                                          //국민주여부
    ...
    etfhares : array[0..9] of char;                           //ETF유통주식수
    etfdnetasset : array[0..14] of char;                      //ETF유통순자산총액
    etfnetasset : array[0..14] of char;                       //ETF순자산총액
    etflastnetasset : array[0..8] of char;                    //ETF최종순자산가치
    etfdnetassetf : array[0..14] of char;                     //ETF외화유통순자산총액(외화)
    etfnetassetf : array[0..14] of char;                      //ETF외화순자산총액
    etflastnetassetf : array[0..8] of char;                   //ETF외화최종순자산가치
    etftypecode : char;                                       //ETF구분코드
    etfcuqty : array[0..7] of char;                           //ETF CU수량
    etfissueqty : array[0..3] of char;                        //ETF구성종목수
    etfindexchgrate : array[0..10] of char;                   //ETF관련지수대비비율
  ...
  TElwMaster = packed record
    ...
    elwundmarket : char;                                      //ELW기초자산시장구분코드
    ...
    elwlpqty : array[0..14] of char;                          //ELWLP보유수량
  ...
  TLpMaster = packed record
    ...
    limitcode : char;                                         //제한단위구분코드
    spreadvalue : array[0..20] of char;                       //호가스프레드값
    restspreadvalue : array[0..20] of char;                   //휴장호가스프레드값
