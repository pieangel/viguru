...
uses Classes, SysUtils, Windows, Dialogs,
  ...
  CleQuoteBroker, CleSymbols, CleQuoteParserIF,
    // Lemon: utilities
  CleParsers,  GleConsts,  UUdpPacketpv4,
  CleKrxSymbols, CleOtherData,
  CleFQn;

const
  MARKET_DEPTH_SIZE = 5;
  CloseSingle = '30';    // 3:15 종가 발생시
  TradeSingle = '20';
  OpenSingle = '10';     // 장전 동시호가
  SisConnected  = '40';

type
  TQuoteParserPv4 = class( TIFQuoteParser )
  private
      // parse
    ...
    procedure ParseCommodityFuturesMarketDepth(stData : String);
    procedure ParseCommodityFuturesTimeNSale(stData : String);
    procedure ParseCommodityFuturesTimeNSaleNMarketDepth( stData : string );
    ...
    procedure ParsePriceLimitFut(stData: string);
    procedure ParsePriceLimitOpt(stData: string);
...
constructor TQuoteParserPv4.Create;
begin
  inherited;
end;

destructor TQuoteParserPv4.Destroy;
begin
  inherited;
end;

procedure TQuoteParserPv4.Parse(stData: String);
var
...
begin
  ...
  if FParseCount < 10 then
    gEnv.EnvLog( WIN_SIS, stData);

  //stData:= Copy(stData, 14,Length(stData));  // VFEP에서 시세 받을경우 해줘야됨.....
  ...
  try
    ...
    begin
      ...
      else if pComm.MarketDiv = Commodity then
        ParseCommodityFuturesTimeNSale( stData )
      else if (pComm.MarketDiv = IndexDer) and ( ( stInfo = FutInfo) or ( stInfo = MiniFutInfo )) then
        ParseKOSPI200FuturesTimeNSale( stData )
      else if (pComm.MarketDiv = IndexDer) and ( ( stInfo = OptInfo) or ( stInfo = MiniOptInfo )) then
    ...
    begin
      ...
      if pComm.MarketDiv = StockNElw then
        ...
        ParseCommodityFuturesMarketDepth( stData )
      else if (pComm.MarketDiv = IndexDer) and (( stInfo = FutInfo) or ( stInfo = MiniFutInfo ))  then
    ...
    begin
      ...
      if pComm.MarketDiv = StockDer then
        ...
        ParseCommodityFuturesTimeNSaleNMarketDepth( stData )
      else if (pComm.MarketDiv = IndexDer) and ( ( stInfo = FutInfo) or ( stInfo = MiniFutInfo )) then
    ...
    if stTR = TrPriceLimit then
    begin
      if stInfo = FutInfo then
        ParsePriceLimitFut(stData)
      else if stInfo = OptInfo then
        ParsePriceLimitOpt(stData)   ;
    end
    else
      Exit;
...
procedure TQuoteParserPv4.ParsePriceLimitFut( stData : string );
var
  vData : PFutPriceLimit;
  stcode: string;
  aSymbol : TSymbol;
  dHigh, dLow : double;
begin
  if Length( stData ) < sizeof( TFutPriceLimit ) then
    Exit;

  vData := PFutPriceLimit( stData );
  ...
  try
    ...
    if aSymbol <> nil then
    begin
      aSymbol.LimitIndex[0] := StrToInt( string( vData.PriceExUpLv ));
      aSymbol.LimitIndex[1] := StrToInt( string( vData.PriceExDownLv ));

      //aSymbol.LimitHigh  := aSymbol.LimitHighArr[ aSymbol.LimitIndex[0] ] ;
      //aSymbol.LimitLow   := aSymbol.LimitLowArr[  aSymbol.LimitIndex[1] ] ;

      dHigh := StrToInt( string(vData.highlimit)) / 100;
      dLow  := StrToInt( string(vData.lowlimit))  / 100;

      if vData.signhigh = '-' then
        aSymbol.LimitHigh := -dHigh
      else
        aSymbol.LimitHigh := dHigh;

      if vData.signlow = '-' then
        aSymbol.LimitLow := -dLow
      else
        aSymbol.LimitLow := dLow;   

      gEnv.EnvLog( WIN_TEST,
        Format( '선물 가격제한폭 변동 %s %.2f, %.2f, %.2f || %.2f, %.2f, %.2f -> %.2f(%d) , %.2f(%d) ', [
            aSymbol.ShortCode,
            ...
            aSymbol.LimitHigh, aSymbol.LimitIndex[0], aSymbol.LimitLow, aSymbol.LimitIndex[1]
             ])
          );
    end;
...
procedure TQuoteParserPv4.ParsePriceLimitOpt( stData : string );
var
  vData : POptPriceLimit;
  stcode: string;
  aSymbol : TSymbol;
  dHigh, dLow : double;
begin
  if Length( stData ) < sizeof( TOptPriceLimit ) then
    Exit;

  vData := POptPriceLimit( stData );
  ...
  try
    ...
    begin
      ...
      gEnv.EnvLog( WIN_TEST,
        Format( '옵션 가격제한폭 변동 %s %.2f, %.2f, %.2f || %.2f, %.2f, %.2f -> %.2f(%d) , %.2f(%d) ', [
            aSymbol.ShortCode,
            aSymbol.LimitHighArr[0], aSymbol.LimitHighArr[1], aSymbol.LimitHighArr[2],
            aSymbol.LimitLowArr[0],  aSymbol.LimitLowArr[1],  aSymbol.LimitLowArr[2],
            aSymbol.LimitHigh, aSymbol.LimitIndex[0], aSymbol.LimitLow, aSymbol.LimitIndex[1]
...
procedure TQuoteParserPv4.ParseCommodityFuturesMarketDepth(stData: String);
var
  aQuote: TQuote;
  dtQuote: TDateTime;
  vData : PCmdFutPrice;
...
procedure TQuoteParserPv4.ParseCommodityFuturesTimeNSale(stData: String);
var
  aQuote: TQuote;
  aSale: TTimeNSale;
...
begin
  ...
  if Length( stData ) < LenCmdFutTick then
    Exit;

  vData := PCmdFutTick( stData );
...
procedure TQuoteParserPv4.ParseCommodityFuturesTimeNSaleNMarketDepth(
  stData: string);
var
  aQuote: TQuote;
  aSale: TTimeNSale;
  stCode, stTime : string;
  vData : PCmdFutTickPrice ;
  iDailyVol, iSize, i : integer;
begin

  iSize := Length( stData );
  if iSize < LenCmdFutTickPrice then
    Exit;

  vData := PCmdFutTickPrice( stData );
...
procedure TQuoteParserPv4.ParseCPPPIIndex(stData: string);
var
...
procedure TQuoteParserPv4.ParseElwGreeks(stData: string);
var
  vData : PElwGreeks;
  stCode  : string;
  aQuote  : TQuote;
...
procedure TQuoteParserPv4.ParseFKOSPIIndex(stData: string);
var
  vData : PIndexData;
  stTime : string;
  aTime  : TDateTime;
...
procedure TQuoteParserPv4.ParseFutOpenInterset(stData: string);
var
...
begin
  ...
  try
    ...
    if string(vData.OIDiv) = 'M0' then   // 전일확정
...
procedure TQuoteParserPv4.ParseK200RiskIndex(stData: string);
var
  vData : PIndexData;
  stDiv, stTime : string;
  aTime  : TDateTime;
...
procedure TQuoteParserPv4.ParseKOSPI200FuturesMarketDepth(stData: String);
var
  aQuote: TQuote;
  dtQuote: TDateTime;
  vData : PFutPrice;
...
procedure TQuoteParserPv4.ParseKOSPI200FuturesTimeNSale(stData: String);
var
  aQuote: TQuote;
  aSale: TTimeNSale;
  stCode, stTime : string;
...
procedure TQuoteParserPv4.ParseKOSPI200FuturesTimeNSaleNMarketDepth(
  stData: string);
var
  aQuote: TQuote;
  aSale: TTimeNSale;
...
procedure TQuoteParserPv4.ParseKOSPI200Index(stData: String);
var
  aQuote: TQuote;
  dtQuote: TDateTime;
  vData : pIndexData;
...
procedure TQuoteParserPv4.ParseKOSPI200IndexExpect(stData: String);
var
  aQuote: TQuote;
  dtQuote: TDateTime;
...
procedure TQuoteParserPv4.ParseKOSPI200OptionMarketDepth(stData: String);
var
  aQuote: TQuote;
  dtQuote: TDateTime;
  vData : POptPrice;
...
procedure TQuoteParserPv4.ParseKOSPI200OptionsTimeNSaleNMarketDepth(
  stData: string);
var
  aQuote: TQuote;
  aSale: TTimeNSale;
...
procedure TQuoteParserPv4.ParseKOSPI200OptionTimeNSale(stData: String);
var
  aQuote: TQuote;
  aSale: TTimeNSale;
  stCode, stTime : string;
...
procedure TQuoteParserPv4.ParseKOSPI200StockFuturesTimeNSaleNMarketDepth(
  stData: string);
var
  aQuote: TQuote;
  aSale: TTimeNSale;
...
procedure TQuoteParserPv4.ParseKospiStockVolumeTotal(stData: String);
var
  aQuote : TQuote;
  dtQuote : TDateTime;
  i : Integer;
...
procedure TQuoteParserPv4.ParseKRXELWMarketDepth(stData: String);
var
  aQuote : TQuote;
  dtQuote : TDateTime;
  i : Integer;
...
procedure TQuoteParserPv4.ParseKRXStockMarketDepth(stData: String);
var
  aQuote : TQuote;
  dtQuote : TDateTime;
  i : Integer;
...
procedure TQuoteParserPv4.ParseKRXStockTimeNSale(stData: string);
var
  aQuote : TQuote;
  dtQuote : TDateTime;
  i : Integer;
...
procedure TQuoteParserPv4.ParseOptOpenInterset(stData: string);
var
...
procedure TQuoteParserPv4.ParseOptRecovery(stData: string);
var
...
procedure TQuoteParserPv4.Parser(pData: PChar; iSize: Integer);
var
  pComm : PCommonHeader;
begin
  Inc(FParseCount);
...
procedure TQuoteParserPv4.ParserOptionGreeks(stData: string);
var
  vData : POptionGreeks;
  aSymbol :  TOption;
begin
...
procedure TQuoteParserPv4.ParseStockFuturesMarketDepth(stData: string);
var
  aQuote : TQuote;
  i : integer;
  dtQuote : TDateTime;
...
procedure TQuoteParserPv4.ParseStockFuturesTimeNSale(stData: string);
var
  aQuote: TQuote;
  aSale: TTimeNSale;
  vData : PStockFutTick;
...
procedure TQuoteParserPv4.ParseVKOSPIIndex(stData: string);
var
  vData : PIndexData;
  stTime : string;
  aTime  : TDateTime;
