  ...
  begin
    ...
    begin
      ...
      begin
        ...
        gEnv.EnvLog(  WIN_VIR,
          format('원주문 찾기 실패 : %s, %s, %d', [ aOrder.Account.Code, aOrder.Symbol.ShortCode, aOrder.OriginNo] ));
        ...
        begin
          aOrder.RejectCode := '999';
          gEnv.EnvLog(  WIN_VIR,
          format('정정 취소 수량 없음 : %s, %s, %d, %d, %d(%d)', [ aOrder.Account.Code, aOrder.Symbol.ShortCode,
          ...
          gEnv.EnvLog(  WIN_VIR,
          format('정정 취소 성공 : %s, %s, %d, %d, %d(%d)', [ aOrder.Account.Code, aOrder.Symbol.ShortCode,
            aOrder.ActiveQty, aTarget.ActiveQty, aOrder.OrderNo, aOrder.OriginNo] ));
...
begin
  ...
  begin
    ...
    begin
      ...
      gEnv.EnvLog( WIN_VIR,
        Format('AcceptSend : %s, %s, %d (시세시각 : %s,  접수시각 : %s)', [ aOrder.Account.Code, aOrder.Symbol.ShortCode, aOrder.OrderNo,
...
begin
  ...
  begin
    ...
    begin
      ...
      stLog := Format('%s, 시세시각:%s, 접수될시각:%s, 주문만든시각:%s, sp : %d, %d', [
        aOrder.Symbol.ShortCode,
        FormatDateTime('hh:nn:ss.zzz', aQuote.LastQuoteTime),
        FormatDateTime('hh:nn:ss.zzz', acptTime),
        FormatDateTime('hh:nn:ss.zzz', aOrder.SentTime ),
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////
    ...
    begin
      ...
      case aOrder.PriceControl of
        pcMarket:
          begin
            gEnv.EnvLog( WIN_VIR, Format('시장가체결 : %s, %s ', [ FormatDateTime('hh:nn:ss.zzz', GetQuoteTime), aOrder.Represent2])
            );
            MakeFill( aOrder );
            NewOrders.Delete(iIndex);
            Continue;
          ...
          begin
            gEnv.EnvLog( WIN_VIR, Format('호가의 조건 충족 체결 : : %s, %s ', [ FormatDateTime('hh:nn:ss.zzz', GetQuoteTime), aOrder.Represent2])
            );
            MakeFill( aOrder );
            NewOrders.Delete(iIndex);
            Continue;
      ...
      case aOrder.PriceControl of
        pcMarket:
          ...
          if (( aOrder.Side > 0 ) and ( (aOrder.Price+0.001) >= FQuote.Asks[0].Price ) and ( FQuote.Asks[0].Price > 0.01) ) or
            (( aOrder.Side < 0 ) and ( aOrder.Price <= (FQuote.Bids[0].Price+0.001) ))  then
          begin
            gEnv.EnvLog( WIN_VIR, Format('체결의 조건 충족 체결 : : %s, %s ', [ FormatDateTime('hh:nn:ss.zzz', GetQuoteTime), aOrder.Represent2])
            );
            MakeFill( aOrder );
            NewOrders.Delete(iIndex);
            Continue;
            ...
            begin
              ...
              begin
                gEnv.EnvLog( WIN_VIR, Format('주문가 누적체결 조건 충족 체결발 : : %s, %s ',
