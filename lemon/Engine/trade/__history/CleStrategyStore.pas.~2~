...
begin
  ...
  begin
    ...
    begin
      aP := aPut.Items[i];
      if (aP.Quote as TQuote).Sales.Count <= 2 then
        exit;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (aQuote.Symbol = gEnv.Engine.SymbolCore.Futures[0]) and (aItem.FutQuote) then  // 선물은 거래되는 종목이 아닌 전략
        aItem.QuoteProc(aQuote, iDataID);
    end;
  end;
end;
