...
type
  ...
  private
    ...
    procedure UpdateSymbol( aSignal : TORSignalItem; aSymbol : TSymbol );
...
begin
  ...
  if not FStccBroker.Connected then
    ShowMessage('MultiChart를 연결하지 못했습니다.')
  else
  begin
    FLastTime := Now;
...
procedure TOmegaIF.UpdateSymbol(aSignal: TORSignalItem; aSymbol: TSymbol);
begin
  if (aSignal = nil) or ( aSymbol = nil ) then Exit;

  if Assigned( FOnSymbolChange ) then
    FOnSymbolChange( Self, aSignal, aSymbol );
end;
...
begin
  ...
  try
    ...
    if ( aDlg.UseLinkSymbolUpdate ) and ( aDlg.LinkSymbol <> nil ) then
      UpdateSymbol( aSignal, aDlg.LinkSymbol );
    // save
    SaveSignals;
    //
...
begin
  ...
  // logging
  gEnv.DoLog(WIN_MC ,'MultiChart >> New Fill(' + DescribeFill(aFill) + ')');
  //aFill.
  ...
  begin
    //..logging
    gEnv.DoLog(WIN_MC, 'OmegaIF >> failed to parse order quantity');

    Exit;
  end;
  iQty := iSign * iQty;
  // logging
  gEnv.DoLog(WIN_MC, 'OmegaIF >> New Order(' + DescribeFill(aFill) + '/Q:' + IntToStr(iQty)+ ')');
...
begin
  ...
  try
    ...
    begin
      gEnv.DoLog(WIN_MC, 'OmegaIF 신호표복구 시스템 신호표가 잘못되었습니다.');
      Exit;
    end;
