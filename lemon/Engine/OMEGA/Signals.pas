...
type
  ...
  private
    ...
    procedure SymbolChange(Sender : TSystemIF; aSignal : TSignalItem; aSymbol : TObject);
...
begin
  ...
  aSystemIF.OnSymbolChange   := SymbolChange;
...
procedure TSignals.SymbolChange(Sender: TSystemIF; aSignal: TSignalItem;
  aSymbol: TObject);
begin
  if (Sender = nil) or (aSignal = nil) or ( aSymbol = nil ) then Exit;

  if Assigned( FOnSignalChange ) then
    FOnSignalChange( Sender, aSignal, aSymbol );
end;
