...
{$REGION '시세 자동 업데이트....'}
  ...
  TAutoSymbolPrice = record
    Header    : TCommonHeader;
		...
		DecimalPrice	: char;                     //	/* Declmal 정보													*/
		Time				  : array [0..7] of char;     //	/* 시간(HH:MM:SS) 												*/
		CurrPrice			: array [0..9] of char;     //	/* 현재가S9(4)V9(2)												*/
		CmpSign				: char;                     //	/* 대비부호
                                              //    0.보합 1.상한 2.하한 3.상승 4.하락
                                              //    5.기세상한 6.기세하한 7.기세상승 8.기세하락					*/
		CmpPrice			: array [0..9] of char;     //	/* 전일대비9(5)V9(2)											*/
		CmpRate				: array [ 0..7] of char;    //	/* 등락율9(5)V9(2) 												*/
		StartPrice		: array [0..9] of char;     //	/* 시가S9(4)V9(2)												*/
		HighPrice			: array [0..9] of char;     //	/* 고가S9(4)V9(2) 												*/
		LowPrice			: array [0..9] of char;     //	/* 저가S9(4)V9(2) 												*/
		SellPrice			: array [0..9] of char;     //	/* 매도호가S9(4)V(2)											*/
		BuyPrice			: array [0..9] of char;     //	/* 매수호가S9(4)V(2)											*/
		NowVol				: array [0..5] of char;     //	/* 체결수량9(6) 												*/
		Volume				: array [0..7] of char;     //	/* 거래량9(8) 													*/
		Money				  : array [0..11] of char;    //	/* 거래대금(천원)9(12)											*/
		PDEndPrice		: array	[0..7] of char;     //	/* 전일종가S9(4)V9(2)											*/
		MarketStat		: array	[0..19] of char;    //	/* 장운영구분
                                              //    1.전체거래중단 2.전체거래재개 3.종목거래중단 4.종목거래재개
                                              //    5.CB중단  6.CB재개 7.장마감 9.가격급변(CB)중단예고
                                              //    A.주식CB중단예고 B.주식CB중간 C.주식CB재개
                                              //    D.종가 단일가 호가 접수개시(스프레드종목은 거래종료)		*/
		NonSettlQty		: array	[0..7] of char;     //	/* 미결제약정수량9(8)											*/
		CmpNonSettlQty: array	[0..7] of char;     //	/* 전일증감9(8)													*/
		TheoryPrice		: array	[0..10] of char;    //	/* 이론가9(4)V9(6) 												*/
		TheoryBasis		: array	[0..7] of char;     //	/* 이론베이시스S9(4)V9(2)										*/
		MarketBasis		: array	[0..7] of char;     //	/* 시장베이시스S9(4)V9(2)										*/
		Disparate			: array [0..8] of char;     //	/* 괴리율9(5)V9(2)												*/
		DispaGap			: array [0..7] of char;     //	/* 괴리폭S9(4)V9(2)												*/
		Delta				  : array [0..12] of char;    //	/* 델타S9(5)V9(4)												*/
		Gamma				  : array [0..12] of char;    //	/* 감마S9(5)V9(4)												*/
		Rho					  : array [0..12] of char;    //	/* 로  S9(5)V9(4)												*/
		Theta				  : array [0..12] of char;    //	/* 쎄타S9(5)V9(4)												*/
		Vega				  : array [0..12] of char;    //	/* 베가S9(5)V9(4)												*/
		HistVolat			  : array [0..7] of char;   //	/* 역사적변동성S9(3)V9(4)										*/
		ListedHighPrice	: array	[0..9] of char;   //	/* 상장중최고가S9(4)V9(2)										*/
		ListedHighPriceDate: array [0..7] of char;//	/* 상장중최고일YYYYMMDD											*/
		ListedLowPrice		 : array [0..9] of char;//	/* 상장중최저가S9(4)V9(2)										*/
		ListedLowPriceDate : array [0..7] of char;//	/* 상장중최저일YYYYMMDD											*/
		ImpliedVolatility	 : array [0..6] of char;//	/* 내재변동성9(3)V9(3)											*/
		RTime_Hi			    : array [0..9] of char;	//  /* 실시간상한가													*/
		RTime_Lo			    : array [0..9] of char;	//  /* 실시간하한가													*/
		RTime_Clear			  : char;                 //	/* 0.비해당종목	1.해당종목 2.일시해제 3.미적용중				*/
		HighLimitPrice		: array [0..9] of char;	//  /* 상한가														*/
		LowLimitPrice		  : array [0..9] of char;	//  /* 하한가
  end;
  ...
  TAutoSymbolTickSub = record
		...
		CmpSign			 : char;                    //	/* 대비부호
                                            //    0.보합 1.상한 2.하한 3.상승 4.하락
                                            //    5.기세상한 6.기세하한 7.기세상승 8.기세하락					*/
		CmpPrice		 : array [0..10] of char;   //	/* 전일대비9(5)V9(2)											*/
		CurrPrice		 : array [0..10] of char;   //	/* 현재가S9(4)V9(2)												*/
		CmpRate			 : array [0..7] of char;    //	/* 등락율9(5)V9(2)												*/
		Volume			 : array [0..7] of char;    //	/* 거래량 														*/
		Money				 : array [0..11] of char;   //	/* 거래대금(천원)												*/
		NowVol			 : array [0..5] of char;    //	/* 체결수량														*/
		NonSettlQty	 : array [0..5] of char;    //	/* 미결제약정수량												*/
		MatchKind		 : char;                    //	/* 현재가의 호가구분 (+.매수 -.매도)							*/
		LastJisu		 : array [0..10] of char;   //	/* KOSPI200지수 종가9(4)V9(2)									*/
		Dungrak			 : char;                    //	/* 등락구분	(' '.보합 +.상승 -.하락)							*/
		Debi				 : array [0..7] of char;    //	/* 등락폭 9(4)V9(2)												*/
		TheoryPrice	 : array [0..10] of char;   //	/* 이론가9(4)V9(6)												*/
		TheoryDungrak: char;                    //	/* 등락구분	(' '.보합 +.상승 -.하락)							*/
		TheoryDebi	 : array [0..7] of char;    //	/* 등락폭9(4)V9(6)												*/
		TheoryBasis	 : array [0..7] of char;    //	/* 이론베이시스S9(4)V9(2)										*/
		MarketBasis	 : array [0..7] of char;    //	/* 시장베이시스S9(4)V9(2)										*/
  end;
...
{$REGION '주문....'}
  ...
  TNewOrderPacket = record
    ...
    User_div      : array [0..2] of char;     //  / IP 뒷자리
    User_Field		: array [0..6] of char;     //	/* 사용자 역역													*/
  end;
