...
uses
  ...
  ApiConsts in '..\Api\Nh\ApiConsts.pas',
  ApiPacket in '..\Api\Nh\ApiPacket.pas',
  CleApiManager in '..\Api\Nh\CleApiManager.pas',
  CleApiReceiver in '..\Api\Nh\CleApiReceiver.pas',
  CleKRXOrderBroker in '..\Api\Nh\CleKRXOrderBroker.pas',
  CleKrxSymbolMySQLLoader in '..\Api\Nh\CleKrxSymbolMySQLLoader.pas',
  ClePriceCrt in '..\Api\Nh\ClePriceCrt.pas',
  NhDataMenu in '..\Api\Nh\NhDataMenu.pas' {NhDataModule1: TDataModule},
  FFundName in '..\Api\Nh\FFundName.pas' {FrmFundName},
  FLogIn in '..\Api\Nh\FLogIn.pas' {FrmLogin},
  FOrpMain in '..\Api\Nh\FOrpMain.pas' {OrpMainForm},
  FServerMessage in '..\Api\Nh\FServerMessage.pas' {FrmServerMessage},
  FAccountPassWord in 'main\FAccountPassWord.pas' {FrmAccountPassWord},
  FQryTimer in '..\Api\Nh\FQryTimer.pas' {FrmQryTimer},
  CleApiRecvThread in '..\Api\Nh\CleApiRecvThread.pas',
  CleJarvisMark2 in 'Order\HULT\CleJarvisMark2.pas',
  FJarvisMark2 in 'Order\HULT\FJarvisMark2.pas' {FrmJarvisMark2},
  UOscillatorBase in 'Order\PDT\UOscillatorBase.pas',
  UOscillators in 'Order\PDT\UOscillators.pas',
  FJikJinHult in 'Order\HULT\FJikJinHult.pas' {FrmJikJinHult},
  CleJikJinHult in 'Order\HULT\CleJikJinHult.pas',
  WRAXLib_TLB in '..\ocx\nh\WRAXLib_TLB.pas',
  HDFCommAgentLib_TLB in '..\ocx\vi\HDFCommAgentLib_TLB.pas',
  MyCollection in '..\Api\Nh\MyCollection.pas';
...
//  bNeedReg  := true;
  ...
  FApp      := 0;
  try
    stClass := ComObj.ClassIDToProgID(CLASS_HDFCommAgent);
  ...
  try
    if bNeedReg then
    begin
      if SysUtils.FileExists(  ExtractFilePath( paramstr(0) )+'WRAX.ocx' ) then
      begin
        // 시스템 레지스터에  multicid.ocx 를 등록하라
        ComObj.RegisterComServer( ExtractFilePath( paramstr(0) )+'WRAX.ocx' );
      end;
    end else
      FApp := 0;

  except
    ShowMessage('WRAX.ocx 설치 실패 ');
    Application.Terminate;
  end;

  Application.Title := 'NHGuruApi';
  Application.CreateForm(TOrpMainForm, OrpMainForm);
  Application.CreateForm(TFPopupMsg, FPopupMsg);
  Application.CreateForm(TNhDataModule1, NhDataModule1);
