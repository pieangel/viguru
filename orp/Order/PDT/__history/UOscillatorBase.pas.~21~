...
begin
  ...
  if FParas.Count = 0 then
    gEnv.EnvLog( WIN_UPDOWN, ParaToStringTitle );
...
procedure TParabolicSignal.ReadParaData;
var
  stTmp : string;
  dtDate : TDateTime;
  i : integer;
begin
  with gEnv do
  begin
    if MySQLConnector = nil then Exit;
    ...
    if not MySQLConnector.Connected then
    begin
      //DoLog('Database connection failed');
      Exit;
    end;

    try

      with MySQLConnector.DataSet do
      begin
        close;
        SQL.Clear;
        stTmp := FormatDateTime('yyyy-mm-dd', gEnv.AppDate );
        //전일 데이터 이용할때
        //SQL.Add( 'Select top 1 Today from tb_Parabolic where Today <:dtToday order by Today desc' );
        //당일 장중 재접속일때
        SQL.Add( 'Select top 1 Today from tb_Parabolic where Today =:dtToday order by Today desc' );
        Parameters.ParamValues['dtToday'] := stTmp;
        Open;
        dtDate := 0;
        while not Eof do
        begin
          dtDate := FieldByName('ToDay').AsDateTime;
          DspLog( Format('Prev Trade Date : %s (%s)', [ FormatDateTime('yyyy-mm-dd', dtDate ),
            FormatDateTime( 'yyyy-mm-dd', gEnv.AppDate ) ] ) );
          break;
        end;
        ...
        begin
          close;
          SQL.Clear;
          stTmp := FormatDateTime('yyyy-mm-dd',dtDate );
          SQL.Add( 'Select Top 1 * from tb_Parabolic where Today=:dtToday and Code=:stCode Order by EndTime desc' );
          Parameters.ParamValues['dtToday'] := stTmp;
          Parameters.ParamValues['stCode'] := Symbol.Code;
          Open;

          while not Eof do
          begin

            LastItem  := Paras.New( FieldByName('ToDay').AsDateTime + FieldByName('EndTime').AsDateTime);
            LastItem.O  := FieldByName('O').AsFloat;
            LastItem.H  := FieldByName('H').AsFloat;
            LastItem.L  := FieldByName('L').AsFloat;
            LastItem.C  := FieldByName('C').AsFloat;

            LastItem.SAR      := FieldByName('SAR').AsFloat;
            LastItem.HighValue:= FieldByName('HighValue').AsFloat;
            LastItem.LowValue := FieldByName('LowValue').AsFloat;
            LastItem.Pos      := FieldByName('Pos').AsInteger;
            LastItem.AF       := FieldByname('AF').AsFloat;

            DspLog( LastItem.ParaToString );
            Next;
          end;
        end;
        Close;
      ...
      MySQLConnector.Disconnect( true );
    end;
...
procedure TParabolicSignal.Save;
var
  i : integer;
  aQuote : TQuote;
  stQuery : string;
...
begin
  ...
  begin
    ...
    end;
   {
    ...
    try
      MySQLConnector.DataSet.Close;
      MySQLConnector.DataSet.SQL.Clear;
      MySQLConnector.DataSet.SQL.Add('delete from tb_Parabolic where today =:dtData');
      MySQLConnector.DataSet.Parameters.ParamValues['dtData']  := FormatDateTime('yyyy-mm-dd', gEnv.AppDate );

      try
        MySQLConnector.DataSet.ExecSQL;
      except
       // DoLog('Master deletion failure');
      end;
    ...
    try
      ...
      if i>=0  then
      begin
        aPara  := FParas.ParaItems[i];
        if aPara = nil then Exit;

      //  if Trunc(aPara.T) < Trunc(  gEnv.AppDate ) then
      //    Continue;

        stQuery := Format( 'insert into tb_Parabolic(' +
                           ' Today,' +
                           ' EndTime,' +
                           ' code,' +
                           ' O,' +
                           ...
                           [
                             FormatDateTime('yyyy-mm-dd',  gEnv.AppDate ),
                             FormatDateTime('hh:nn:ss',   aPara.T ) ,
                             aQuote.Symbol.Code,
                             aPara.O,
                             aPara.H,
                             aPara.L,
        ...
        try
          MySQLConnector.Conn2.Execute( stQuery );
        except
          gEnv.EnvLog( WIN_TEST, stQuery );
        end;
      end;
