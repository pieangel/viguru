...
begin
  ...
  begin
    DoLog( Format('한도  %.0f < %.0f ', [ dPL / 10000, FParam.LiskAmt ]));
    Result := true;
  end;
...
begin
  ...
  begin
    ...
    if iDir <> 0 then
    begin
      DoLog( Format('주문개시 : %s ( %.2f, %.2f, %.2f )', [ ifThenStr( iDir > 0,'상승방향','하락방향'),
  ...
  // 추가진입
    ...
    begin
      ...
      if iDir <> 0 then
      begin
        DoLog( Format('%d. 추가주문 : %s ( %.2f, %.2f, %.2f )', [ FOrderCnt+1, ifThenStr( iDir > 0,'상승방향','하락방향'),
          FSymbol.DayOpen , FSymbol.Last, dGap]));
        // 주문
        DoOrder;
        aCon.Ordered  := true;
