...
begin
  ...
  begin
    ...
    begin
      FilledQty[ptLong] := FilledQty[ptLong] + iFillQty;

      gEnv.EnvLog( WIN_HULT, Format('Price OnOrder Fill : Ask = %d, Bid = %d, %d, %s, FilledQty = %d, %d , %s, %s, %s',
      [ (Collection as TPriceSet).AskOrdCnt, (Collection as TPriceSet).BidOrdCnt, ORderQtySum, aOrder.OrderTypeDesc,
       FilledQty[ptLong],  FilledQty[ptShort], stL, stS, GetPriceDesc]));

      if FilledQty[ptLong] > (Collection as TPriceset).OrderQty then
        gEnv.EnvLog( WIN_HULT, Format('OnOrder Fill 이상 FilledQty = %d, %d , %s, %s, %s ',[FilledQty[ptLong],  FilledQty[ptShort], stL, stS, GetPriceDesc]));


      if (FilledQty[ptLong] >= (Collection as TPriceset).OrderQty) and (FullQty[ptLong]) and (IsSameSide(1)) then
      begin
        FullQty[ptLong] := false;
        FilledQty[ptLong] := 0;
        (Collection as TPriceSet).SetOrdCnt(aOrder, false);
        Result := true;
      end;
    end else if aOrder.Side < 0 then
    begin
      FilledQty[ptShort] := FilledQty[ptShort] + iFillQty;

      gEnv.EnvLog( WIN_HULT, Format('Price OnOrder Fill : Ask = %d, Bid = %d, %d, %s, FilledQty = %d, %d , %s, %s, %s',
      [ (Collection as TPriceSet).AskOrdCnt, (Collection as TPriceSet).BidOrdCnt, ORderQtySum, aOrder.OrderTypeDesc,
       FilledQty[ptLong],  FilledQty[ptShort], stL, stS, GetPriceDesc]));

      if abs(FilledQty[ptShort]) > (Collection as TPriceset).OrderQty then
        gEnv.EnvLog( WIN_HULT, Format('OnOrder Fill 이상 FilledQty = %d, %d , %s, %s, %s ',[FilledQty[ptLong],  FilledQty[ptShort], stL, stS, GetPriceDesc]));

      if (abs(FilledQty[ptShort]) >= (Collection as TPriceset).OrderQty) and (FullQty[ptShort]) and (IsSameSide(-1)) then
      begin
  ...
  end else if (aOrder.State = osCanceled) and ( not IsCancelOrder) then
  begin
    if (aORder.Side > 0) and (FullQty[ptLong]) then
    begin
       FullQty[ptLong] := false;
       FilledQty[ptLong] := 0;
      (Collection as TPriceSet).SetOrdCnt(aOrder, false);
    ...
    if (aORder.Side < 0) and (FullQty[ptShort]) then
    begin
       FullQty[ptShort] := false;
       FilledQty[ptShort] := 0;
      (Collection as TPriceSet).SetOrdCnt(aOrder, false);
     ...
     {
    gEnv.EnvLog( WIN_HULT, Format('Price OnOrder Cancel : Ask = %d, Bid = %d, %d, %s, %s',
      [ (Collection as TPriceSet).AskOrdCnt, (Collection as TPriceSet).BidOrdCnt, ORderQtySum, aOrder.OrderTypeDesc,  GetPriceDesc]));}
{  end;
