...
begin
  ...
  if ( FSymbol.Spec.Market = mtOption) and ( Position.AvgPrice < 3.0-PRICE_EPSILON) then
    ...
    dTickValue := FSymbol.Spec.TickSize;

  // n개 이상 포지션 일때 avgPrice 에 청산주문
  ...
  // 위의 과정으로..청산주문 깔어야 할 기준가격을 구함...     
  gEnv.EnvLog( WIN_SHORTHULT, Format('%s  %.2f 에 체결 -> 포지션 %d (>%d) 개, 평균가 %.2f -> %.2f 에 청산주문', [
    ifThenStr( aOrder.Side > 0 ,'매수','매도'), aOrder.Price,   abs( Position.Volume ),
      ...
      begin
        ...
        begin
          ...
          gEnv.EnvLog(WIN_SHORTHULT, Format('cancel avg liq order(%d), %s, %.2f, %d',
             [i, ifThenStr( iSide > 0, '매수','매도'), pOrder.Price, pOrder.OrderNo  ]))
        end;
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        gEnv.EnvLog(WIN_SHORTHULT, Format('avg liq order(%d), %s, %.2f',[i, ifThenStr( iSide > 0, '매수','매도'), dPrice  ]));
    ...
    begin
      ...
      DoOrder2( FData.OrdQty, iSide, 0, dPrice );
      gEnv.EnvLog(WIN_SHORTHULT, Format('avg liq order(%d), %s, %.2f',[ iCnt+1, ifThenStr( iSide > 0, '매수','매도'), dPrice  ]));
    end;
end;
