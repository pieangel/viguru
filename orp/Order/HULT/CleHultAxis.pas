...
type
  ...
  private
    ...
    FBackColor : TColor;
    FOrgPL: TLogPL;
    FPlatoonFlag: boolean;        // false 면 진입금
...
begin
  ...
  begin
    ...
    begin
      gEnv.EnvLog( WIN_HULT, Format('플래툰 데이타 이상 , 전전일고저 ( %s - %s )  전일고저 (%s - %s )  시가 : %s ',
    ...
    begin
      ...
      gEnv.EnvLog( WIN_HULT,   Format(' (전전일고저) %s ( %s-%s ) < %s (%s - %s) (전일고저)  ( 시가 %s ) : 진입 %s ', [
        ...
        FSymbol.PriceToStr( FSymbol.DayOpen ),   ifThenStr( FPlatoonFlag , '있음','없음')
        ]));
      Exit;
    end;
