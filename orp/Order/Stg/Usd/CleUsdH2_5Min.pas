...
uses
  GAppEnv, GleLib, CleQuoteTimers , FUsdH2_5Min, CleKrxSymbols,
  GleConsts
  ;

{ TUsdH2_5Min_Trend }

procedure TUsdH2_5Min_Trend.Reset;
begin
  //FBongCnt:= 0;
  FEntryPrice:= 0;
  FOrdSide:= 0;
...
procedure TUsdH2_5Min_Trend.CalcRange;
begin
...
constructor TUsdH2_5Min_Trend.Create(aObj: TObject);
begin
  FRun:= false;
...
destructor TUsdH2_5Min_Trend.Destroy;
begin
...
function TUsdH2_5Min_Trend.IsRun : boolean;
begin
...
procedure TUsdH2_5Min_Trend.DoLog(stData: string);
begin
  if FAccount <> nil then
    gEnv.EnvLog( WIN_USDF, stData, false, FAccount.Name );
end;

procedure TUsdH2_5Min_Trend.DoLossCut;
begin
  DoLossCut( nil );
end;

procedure TUsdH2_5Min_Trend.DoLossCut(aQuote: TQuote);
var
  //aPos : TPosition;
  aOrd : TOrder;
  I: Integer;
...
procedure TUsdH2_5Min_Trend.DoOrder(aQuote: TQuote; iDir: integer);
var
  iQty, I: Integer;
begin
...
procedure TUsdH2_5Min_Trend.DoOrder(aQuote: TQuote; aAccount: TAccount;
...
function TUsdH2_5Min_Trend.init(aAcnt: TAccount; aQuoteSymbol,
...
function TUsdH2_5Min_Trend.Start: boolean;
begin
  Result := false;
  ...
  // 20180915  종목정보 요청..
  gEnv.Engine.SendBroker.RequestSymbolInfo2( FQteSymbol );
  //gEnv.Engine.SendBroker.ReqChartData( FQteSymbol, incDay(Date,-1), 1, 0, '1' );

end;

procedure TUsdH2_5Min_Trend.Stop;
begin
  FRun := false;
...
function TUsdH2_5Min_Trend.CheckLossCut( aQuote : TQuote ) : boolean;
var
  dParam, dVal : double;
begin
  Result := false;
...
procedure TUsdH2_5Min_Trend.SymbolPrc(Sender, Receiver: TObject; DataID: Integer;
...
procedure TUsdH2_5Min_Trend.QuotePrc(Sender, Receiver: TObject;
  DataID: Integer; DataObj: TObject; EventID: TDistributorID);
begin
  if ( Receiver <> Self ) or ( DataObj = nil ) then Exit;
...
function TUsdH2_5Min_Trend.CheckLiquid( aQuote : TQuote ) : Boolean;
begin
  Result := false;
...
function TUsdH2_5Min_Trend.CheckOrder( aQuote : TQuote ) : integer;
var
  uQuote : TQuote;
begin
  Result := 0;
...
procedure TUsdH2_5Min_Trend.OnQuote(aQuote: TQuote);
var
  bTerm : boolean;
  dtNow : TDateTime;
  iDir  : integer;
...
procedure TUsdH2_5Min_Trend.TradePrc(Sender, Receiver: TObject; DataID: Integer;
  DataObj: TObject; EventID: TDistributorID);
begin
