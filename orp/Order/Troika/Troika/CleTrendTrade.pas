...
begin
  ...
  begin
    ...
    begin
      ...
      stC := Format('%.0f', [ FParam.OtrCon1[ptLong]]);
      // 콜매수(0.7),
      aQuote := GetOption('C',FParam.BasePrice1[ptLong]);
      if (aQuote <> nil)  and (( stC[1] = '2') or ( stC[1] = '1')) then
      begin
        aOrdRes := GetOrdRes( aQuote, stTrend );
  ...
  begin
    ...
    begin
      ...
      stC := Format('%.0f', [ FParam.OtrCon1[ptShort]]);
      // 콜매도
      ...
      // 풋매수
      aQuote := GetOption('P',FParam.BasePrice1[ptShort]);
      if (aQuote <> nil) and (( stC[1] = '3') or ( stC[1] = '1')) then
      begin
        aOrdRes := GetOrdRes( aQuote, stTrend );
        if aOrdRes.OrdDir = 0 then
        begin
