...
uses
  ...
  DongBuMenu in '..\Api\bongbu\DongBuMenu.pas' {DongBuData: TDataModule},
  ...
  MainMenu in '..\Api\bongbu\MainMenu.pas',
  DONGBUAPILib_TLB in '..\ocx\dongbu\DONGBUAPILib_TLB.pas';
...
begin
  ...
  FApp      := 0;
  try
    stClass := ComObj.ClassIDToProgID(CLASS_DongbuAPI);
    if stClass = '' then bNeedReg := true;    
  except
    bNeedReg  := true;
  end;
  ...
  try
    if bNeedReg then
    begin
      if SysUtils.FileExists(  ExtractFilePath( paramstr(0) )+'DongbuAPI.ocx' ) then
      begin
        // 시스템 레지스터에  multicid.ocx 를 등록하라
        ComObj.RegisterComServer( ExtractFilePath( paramstr(0) )+'DongbuAPI.ocx' );
      end;
    end else
      FApp := 0;

  except
    ShowMessage('DongbuAPI.ocx 설치 실패 ');
    Application.Terminate;
  end;

  Application.Title := '동부국내 GuruApi';
  Application.CreateForm(TOrpMainForm, OrpMainForm);
  Application.CreateForm(TDongBuData, DongBuData);
  Application.CreateForm(TFPopupMsg, FPopupMsg);
  Application.CreateForm(TFrmServerMessage, FrmServerMessage);
  Application.ShowMainForm  := true;
  OrpMainForm.Visible := false;
