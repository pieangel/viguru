...
begin
  ...
  if aItem <> nil then
  case FBase of
    cbTick, cbQuote    : Result := FormatDateTime('hh"시"nn"분"ss"초"', aItem.StartTime);
    cbMin     : Result := FormatDateTime('mm월dd일hh시nn분', aItem.StartTime);
    cbDaily,
    cbWeekly  : Result := FormatDateTime('mm"월"dd"일"', aItem.StartTime);
    cbMonthly : Result := FormatDateTime('yyyy"년"mm"월"', aItem.StartTime);
  end;
{ // 2004.1.12
  if aItem <> nil then
  case FBase of
    cbTick    : Result := FormatDateTime('hh"시"nn"분"ss"초"', aItem.LastTime);
    cbMin     : Result := FormatDateTime('hh"시"nn"분"', aItem.LastTime);
    cbDaily,
    cbWeekly  : Result := FormatDateTime('mm"월"dd"일"', aItem.LastTime);
    cbMonthly : Result := FormatDateTime('yyyy"년"mm"월"', aItem.LastTime);
  end;
}
end;
